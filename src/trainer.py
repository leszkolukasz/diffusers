...
class Trainer:
    ...
    def _get_trainer_state_path(self):
        return os.path.join(self.config.checkpoint_dir, f"trainer_{self.model.file_name}")
    ...
    def load_checkpoint(self):
        ...
        if os.path.exists(path):
            ...
            logger.info(f"Resuming training from epoch {self.current_epoch}, step {self.total_steps_executed}")
        else:
    ...
    def train(self, dataloader: DataLoader):
        ...
        lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=total_training_steps, eta_min=1e-6, last_epoch=self.total_steps_executed - 1
        )
        ...
        for epoch in range(self.current_epoch, self.config.epochs):
            ...
            for batch_idx, (X, _) in enumerate(pbar):
                ...
                if (self.total_steps_executed % self.config.checkpoint_interval_steps == 0):
